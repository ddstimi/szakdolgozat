...
interface Concert extends RowDataPacket {
  ...
  artist_name: string;
  image?: string;
  genre?: string | null; // GROUP_CONCAT result
}
...
const ConcertsModel = {
  async getTopPicks(userId: number): Promise<Concert[]> {
    const limit = TOP_PICKS_LIMIT;
    ...
    const [preferredConcerts] = await pool.query<Concert[]>(
      ...
      LEFT JOIN user_artists ua
        ON ua.artist_id = c.artist_id AND ua.user_id = ?
      LEFT JOIN user_venues uv
        ON uv.venue_id = c.venue_id AND uv.user_id = ?
      LEFT JOIN user_cities uc
        ON uc.city_id = v.city_id AND uc.user_id = ?
      WHERE c.date >= NOW()
        AND c.cancelled = FALSE
        AND (
          ua.user_id IS NOT NULL
          OR uv.user_id IS NOT NULL
  ...
  async getUpcomingByUserId(userId: number): Promise<Concert[]> {
    const [concerts] = await pool.query<Concert[]>(
      `
      SELECT
        c.id,
  ...
  async getPastByUserId(userId: number): Promise<Concert[]> {
    const [concerts] = await pool.query<Concert[]>(
      `
      SELECT
        c.id,
  ...
  async getPopular(): Promise<Concert[]> {
    const limit = POPULAR_LIMIT;
    const map = new Map<number, Concert>();
    ...
    if (map.size >= TOP_PICKS_LIMIT) {
      return Array.from(map.values());
    }

    const remaining = limit - map.size;
